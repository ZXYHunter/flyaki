...
function consultantBasicInfoSubmit() {
    ...
    for(var i=1;i<=count;i++){
        ...
        result+=" <div style='width: 90%;'> <a id='consultantDisplay_"+idmap[trans[i]]+"' href='/wechatSpace_"+idmap[trans[i]]+"' ;'>";
        result+="<table> <tr> <td style='width: 40%'> <img class='img' src='"+photoaddrmap[trans[i]]+"' style='border-radius: 50%;width: 80%'/> </td>";
        result+="<td style='width: 60%'>";
        result+="<p class='title'>"+namemap[trans[i]]+"</p>";
        result+="<p class='detail'>";
        result+="<span style='font-size:25px; color: rgba(255,64,62,1);' class='glyphicon glyphicon-map-marker'></span><br>"+majormap[trans[i]];
        result+=majormap[trans[i]]+" , "+universitymap[trans[i]];
        result+="</p> </td> </tr>";
        result+="<tr style='text-align: left;'><td colspan='2'><br>";
        result+="<span class='title'>工作经历:<br></span>";
        result+="<p class='detail'>"+expmap[trans[i]]+"</p> <span class='title'>申到offer:<br></span>";
        result+="<p class='detail'>"+majormap[trans[i]]+"</p> </td> </tr> </table> </a></div>";
